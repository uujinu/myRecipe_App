...
export default function SignUpForm() {
  ...
  const validate = (input = values) => {
    const temp = { ...errors };
    if ("email" in input)
      temp.email = /$^|.+@.+..+/.test(input.email)
        ? ""
        : "잘못된 이메일 형식입니다.";
    if ("password1" in input) {
      if (input.password1.length < 8)
        temp.password1 = "비밀번호는 최소 8자입니다.";
      else if (input.password1.length > 15)
        temp.password1 = "비밀번호는 8~15자로 구성되어야 합니다.";
      else {
        temp.password1 = /^(?=.*[a-zA-Z])((?=.*\d)+(?=.*\W)).{8,15}$/.test(
          input.password1,
        )
          ? ""
    ...
    if ("password2" in input) {
      temp.password2 =
        values.password1 !== input.password2
          ? "비밀번호를 동일하게 입력해주세요."
          : "";
    }
    if ("username" in input) temp.username = "";

    setErrors({ ...temp });

    if (temp.password1 === "") setPW(false);
    else setPW(true);

    if (temp.email === "" && temp.password2 === "") setBtn(false);
    else setBtn(true);
  };
